    ...
    public LoomScreenHandler(Inventory playerinventory, LoomBlockEntity loomBlockEntity) {
        ...
        for(int xIndex = 0; xIndex < 2; xIndex++) {
            for(int yIndex = 0; yIndex < 2; yIndex++) {
                addSlot(new Slot(loomBlockEntity, xIndex + yIndex * 2, 8 + xIndex * 18, 43 + yIndex * 18));
            }
        }

        for(int xIndex = 0; xIndex < 2; xIndex++) {
            for(int yIndex = 0; yIndex < 2; yIndex++) {
                addSlot(new Slot(loomBlockEntity, xIndex + yIndex * 2 + 4, 64 + xIndex * 18, 43 + yIndex * 18));
            }
        }

        addSlot(new Slot(loomBlockEntity, 8, 16, 30));
