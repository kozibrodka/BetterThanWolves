...
public class FCEntityWaterWheel extends EntityBase
{

    public FCEntityWaterWheel(Level world)
    {
        super(world);
        bWaterWheelIAligned = true;
        fRotation = 0.0F;
    ...
    public FCEntityWaterWheel(Level world, double x, double y, double z,
                              boolean bJAligned)
    {
        this(world);
        setPosition(x, y, z);
        bWaterWheelIAligned = bJAligned;
        AlignBoundingBoxWithAxis();
    }

    public FCEntityWaterWheel(Level level, Double aDouble, Double aDouble1, Double aDouble2) {
        this(level);
    }

    private void AlignBoundingBoxWithAxis()
    {
        if(bWaterWheelIAligned)
        {
    ...
    {
        nbttagcompound.put("bWaterWheelIAligned", bWaterWheelIAligned);
    ...
    {
        bWaterWheelIAligned = nbttagcompound.getBoolean("bWaterWheelIAligned");
    ...
    public void tick()
    {
        ...
        if(iFullUpdateTickCount <= 0)
        {
            ...
            if(!WaterWheelValidateAreaAroundBlock(level, iCenterI, iCenterJ, iCenterK, bWaterWheelIAligned))
            {
                DestroyWithDrop();
                return;
            }
...
//            }
        ...
        fRotation += fWaterWheelCurrentRotationSpeed;
        if(fRotation > 360F)
        {
            fRotation -= 360F;
        } else
