    ...
    implements RotatableBlock
{
    ...
    public void onEntityCollision(Level world, int i, int j, int k, EntityBase entity)
    {
        if((entity instanceof Arrow) || (entity instanceof BroadheadArrowEntity))
        {
            ...
            if(world.getTileId(targeti, targetj, targetk) == BlockListener.blockDetectorLogic.id)
            {
                BlockListener.blockDetectorLogic.onEntityCollision(world, targeti, targetj, targetk, entity);
            }
        }
    }
    ...
    public boolean PlaceDetectorLogicIfNecessary(Level world, int i, int j, int k)
    {
        ...
        {
            world.setTile(targeti, targetj, targetk, BlockListener.blockDetectorLogic.id);
            return true;
        } else
        {
            return false;
    ...
    public boolean CheckForDetection(Level world, int i, int j, int k)
    {
        ...
        if(targetid > 0)
        {
            if(targetid == BlockListener.blockDetectorLogic.id)
            {
