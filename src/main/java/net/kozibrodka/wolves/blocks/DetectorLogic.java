...
public class DetectorLogic extends TemplateBlockBase
{

    public DetectorLogic(Identifier iid)
    {
    ...
    public void onBlockRemoved(Level world, int i, int j, int k)
    {
        for(int iFacing = 0; iFacing <= 5; iFacing++)
        {
            ...
            if(iTargetid == BlockListener.blockDetector.id)
            {
    ...
    private boolean CheckForNeighbouringDetector(Level world, int i, int j, int k)
    {
        for(int iTempFacing = 0; iTempFacing <= 5; iTempFacing++)
        {
            ...
            if(world.getTileId(tempPos.i, tempPos.j, tempPos.k) == BlockListener.blockDetector.id && ((DetectorBlock)BlockListener.blockDetector).GetFacing(world, tempPos.i, tempPos.j, tempPos.k) == UnsortedUtils.GetOppositeFacing(iTempFacing))
            {
                return true;
            }
        }
