...
public class FCBlockStokedFire extends TemplateFire implements BlockWithWorldRenderer
{
    ...
    public void onScheduledTick(Level world, int i, int j, int k, Random random) {
        ...
        if (world.getTileId(i, j - 2, k) == mod_FCBetterThanWolves.fcBBQ.id) {
            if (((FCBlockBBQ) mod_FCBetterThanWolves.fcBBQ).IsBBQLit(world, i, j - 2, k)) {
                flag = true;
            } else {
                world.setTile(i, j, k, 0);
                return;
