...
public class FCItemWindMill extends TemplateItemBase

{
    public FCItemWindMill(Identifier iItemID)
    {
        super(iItemID);
        maxStackSize = 1;
    }
    ...
    public boolean useOnTile(ItemInstance iteminstance, PlayerBase entityplayer, Level world, int i, int j, int k, int l)
    {
        ...
        if(iTargetid == mod_FCBetterThanWolves.fcAxleBlock.id && !world.isServerSide)
        {
            ...
            if(iAxisAlignment != 0)
            {
                ...
                if(FCEntityWindMill.WindMillValidateAreaAroundBlock(world, i, j, k, bIAligned))
                {
                    world.spawnEntity(new FCEntityWindMill(world, (float)i + 0.5F, (float)j + 0.5F, (float)k + 0.5F, bIAligned));
                    iteminstance.count--;
                    return true;
                }
