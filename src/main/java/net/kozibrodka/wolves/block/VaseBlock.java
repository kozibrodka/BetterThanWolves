    ...
    implements RotatableBlock, BlockWithWorldRenderer, BlockWithInventoryRenderer
{
    ...
    public boolean onUse(World world, int i, int j, int k, PlayerEntity entityPlayer)
    {
        ...
        {
            VaseBlockEntity tileEntityVase = (VaseBlockEntity)world.method_1777(i, j, k);
    ...
    {
        VaseBlockEntity tileEntity = (VaseBlockEntity)world.method_1777(i, j, k);
        if(tileEntity != null)
        {
    ...
    public void BreakVase(World world, int i, int j, int k)
    {
        ...
        Minecraft.class.cast(FabricLoader.getInstance().getGameInstance()).soundManager.method_2015(soundGroup.getSound(), (float)i + 0.5F, (float)j + 0.5F, (float)k + 0.5F, (soundGroup.method_1976() + 1.0F) / 2.0F, soundGroup.method_1977() * 0.8F);
