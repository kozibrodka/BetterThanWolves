...
public class WindMillRenderer extends EntityRenderer {
    ...
    public void render(Entity entity, double d, double d1, double d2,
        ...
        float f2 = (float) fcentitywindmill.getMillTimeSinceHit() - f1;
        float f3 = (float) fcentitywindmill.getMillCurrentDamage() - f1;
        float f4 = 0.0F;
        if (f3 < 0.0F) {
            f3 = 0.0F;
        }
        if (f3 > 0.0F) {
            f4 = ((MathHelper.sin(f2) * f2 * f3) / 40F) * (float) fcentitywindmill.getMillRockDirection();
        }
