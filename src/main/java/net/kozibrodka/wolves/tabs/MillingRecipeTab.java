...
public class MillingRecipeTab extends TabWithTexture {
    ...
    public MillingRecipeTab(Namespace tabCreator) {
        this(tabCreator, new ArrayList<ItemStack[]>(MillingRecipeRegistry.getInstance().getRecipes()), BlockListener.millStone);
    }

    public MillingRecipeTab(Namespace tabCreator, List<ItemStack[]> recipesReady, Block tabBlock) {
        this(tabCreator, 2, recipesReady, tabBlock, "/assets/wolves/stationapi/gui/hmi_tabs/one_in_one_out.png", 140, 56, 22, 15);
    }

    public MillingRecipeTab(Namespace tabCreator, int slotsPerRecipe, List<ItemStack[]> recipesReady, Block tabBlock, String texturePath, int width, int height, int textureX, int textureY) {
    ...
    public void draw(int x, int y, int recipeOnThisPageIndex, int cursorX, int cursorY) {
        ...
        Utils.drawScaledItem(new ItemStack(BlockListener.millStone), x + 54, y + 12, 34);
    }
