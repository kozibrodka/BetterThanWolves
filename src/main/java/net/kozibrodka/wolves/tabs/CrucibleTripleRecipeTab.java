...
public class CrucibleTripleRecipeTab extends TabWithTexture {
    ...
    public CrucibleTripleRecipeTab(Namespace tabCreator) {
        this(tabCreator, new ArrayList<ItemStack[]>(CrucibleCraftingManager.getInstance().getTripleRecipes()), BlockListener.crucible);
    }

    public CrucibleTripleRecipeTab(Namespace tabCreator, List<ItemStack[]> recipesReady, Block tabBlock) {
        this(tabCreator, 4, recipesReady, tabBlock, "/assets/wolves/stationapi/gui/hmi_tabs/three_in_one_out.png", 140, 56, 22, 15);
    }

    public CrucibleTripleRecipeTab(Namespace tabCreator, int slotsPerRecipe, List<ItemStack[]> recipesReady, Block tabBlock, String texturePath, int width, int height, int textureX, int textureY) {
    ...
    public void updateRecipesWithoutClear(ItemStack filter, Boolean getUses) {
        ...
        recipesReady.forEach(recipe -> {
            ...
            ItemStack thirdInput = recipe[3];
            ItemStack output = recipe[0];
            if (filter == null || (!getUses && compare(filter, output)) || (getUses && compare(filter, firstInput)) || (getUses && compare(filter, secondInput)) || (getUses && compare(filter, thirdInput)) || filter.isItemEqual(new ItemStack(tabBlock))) {
                recipes.add(recipe);
            }
        });
        size = recipes.size();
