...
public class CauldronDoubleRecipeTab extends TabWithTexture {
    ...
    public CauldronDoubleRecipeTab(Namespace tabCreator) {
        this(tabCreator, new ArrayList<ItemStack[]>(CauldronCraftingManager.getInstance().getDoubleRecipes()), BlockListener.cauldron);
    }

    public CauldronDoubleRecipeTab(Namespace tabCreator, List<ItemStack[]> recipesReady, Block tabBlock) {
        this(tabCreator, 3, recipesReady, tabBlock, "/assets/wolves/stationapi/gui/hmi_tabs/two_in_one_out.png", 140, 56, 22, 15);
    }

    public CauldronDoubleRecipeTab(Namespace tabCreator, int slotsPerRecipe, List<ItemStack[]> recipesReady, Block tabBlock, String texturePath, int width, int height, int textureX, int textureY) {
    ...
    public void draw(int x, int y, int recipeOnThisPageIndex, int cursorX, int cursorY) {
        ...
        Utils.drawScaledItem(new ItemStack(BlockListener.cauldron), x + 54, y + 12, 34);
    }
    ...
    public void updateRecipesWithoutClear(ItemStack filter, Boolean getUses) {
        ...
        recipesReady.forEach(recipe -> {
            ItemStack firstInput = recipe[1];
            ItemStack secondInput = recipe[2];
            ItemStack output = recipe[0];
            if (filter == null || (!getUses && compare(filter, output)) || (getUses && compare(filter, firstInput)) || (getUses && compare(filter, secondInput)) || filter.isItemEqual(new ItemStack(tabBlock))) {
                recipes.add(recipe);
            }
        });
        size = recipes.size();
